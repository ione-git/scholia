...
- A change to any `@Model`, even a new optional property, adds a new `SchemaV(n+1)` and a migration stage (`.lightweight`, or `.custom` when values must be filled) in `App/Sources/Model/ScholiaSchema.swift`. A store whose checksum is not in the plan fails to open.
  - The new version copies all six models, because relationships bind to classes of the same version. The previous version is never edited again. The typealiases and `Schema(versionedSchema:)` in `Storage` move to the new version; behaviour (`fileURL`, `order`, `makeDefault()`) stays in extensions on the typealiases.
  - The shared value types (`ReadingPosition`, `TimeOfDay`, the stored enums, `DesignSystem.ReaderTheme`) are part of every version's stored shape. Changing their fields changes V1 too, so first freeze a copy of the old type inside the old version.
  - Stored raw values are never renamed or removed. A rename does not change the checksum, so no new version can carry it: reading an old value crashes, and a migration through two versions with the same checksum crashes too.
