...
- `Icon`: line icons drawn from the screens' SVG paths on the 24pt grid (`back`, `add`, `bookmark`, `more`, `chevron`, `check`, `settings`, `search`, `select`, `newCollection`, `sort`). A new icon is a new case with the path from the screen.
...
- `GoalRing(value:size:isActive:center:)` (0…1, capped at 1): `track` ring with an `accent` arc from the top. `.header` is the Home ring, 32pt box, 4pt stroke; at `value` 1 it becomes a 28pt `accent` disc with an `on-accent` check. `.popover` is 56pt with a 6pt stroke and `center` (a `Text` from the app, `title-3` `ink`) inside; at 1 it is a full ring. `isActive` draws the `accent-tint` disc behind it while its popover is open. `Size.width` for layout. Accessibility value is the percentage; the app adds the label (on the Home ring button, which keeps the 32pt layout with a 44pt hit area).
