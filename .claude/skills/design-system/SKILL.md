...
- Sheets: `.modalSheet(isPresented: $isShown) { content }` in place of `.sheet` for modal sheets (`surface`, `radius-sheet`, grabber; `scrim` over the view it is attached to instead of the iOS dimming, a tap on it dismisses, and that view is hidden from accessibility while the sheet is up). The scrim covers only that view, so attach it to the screen's root view. `.glassSheetStyle()` for the reader settings sheet (system glass, no dimming, the page stays interactive). `SheetHeader(Text("Add Book")) { Button("Cancel") { … }.buttonStyle(.sheetCancel) } trailing: { Button("Done") { … }.buttonStyle(.sheetDone) }` (`EmptyView()` for a missing side).
