...
- `Icon`: line icons drawn from the screens' SVG paths on the 24pt grid (`back`, `add`, `bookmark`, `more`, `chevron`, `chevronDown`, `check`, `settings`, `search`, `select`, `newCollection`, `sort`, `collection`, `info`, `trash`). A new icon is a new case with the path from the screen. `Icon.info.menuImage` is the icon as a 20pt template `Image` with the 2pt stroke, for system menus that draw only images (`.contextMenu`): `Label { Text("Book Info") } icon: { Icon.info.menuImage }`.
...
- `BookCover(title:author:color:image:size:isFinished:finishedValue:)`: sizes `.thumbnail` 40x60, `.row` 80x120, `.grid` 100x150, `.library` 107x152, `.hero` 160x240 and `.heroLarge` 180x270 (hero glow), `Size.width` / `.height` for layout. `color` is the generated block and the hue of the hero glow, so pass it with an `image` too. A title word too long for the cover shrinks the title instead of breaking mid-word. The cover is one accessibility element labelled with the title; `finishedValue` (e.g. `Text("Finished")`) is its accessibility value while `isFinished`. `ProgressBar(value:)` (0…1) is the Home bar: give it the cover's width. `BookCover.generatedColor(for: title)` is the block colour for a book without cover art: one of the screens' cover colours, stable per title. `.selectable(isSelected:)` is the select-mode cover (Library-Select): a 2pt `accent` ring 3pt outside the cover while selected, and a 24pt checkbox 6pt in from the bottom-trailing corner, `accent` with an `on-accent` check and a 2pt `on-accent` ring when selected, else a light `scrim` disc with a 1.5pt light `glass-border` ring (fixed light values, it sits on the cover). It keeps the cover's size and hides the checkbox from accessibility: wrap the cover in a `Button` and add the `.isSelected` trait.
...
- `TranslationBubble(word: Text(verbatim: word), phase: .loading(label: Text("Translating \(word)")), wordLocale:, translationLocale:, identifier: "reader.bubble")`: the word tap bubble, 236pt wide, padding 12/14, `.glassEffect` tinted `surface-glass-strong` in `radius-xl`, no shadow. `phase`: `.loading(label:)` (132x14 `track` bar with a shimmer and three `accent` dots, still under Reduce Motion; the label is the VoiceOver label), `.translated(translation:ipa:grammar:)` (IPA right of the word, `translation` row, grammar line with an `ink-faint` chevron; `grammar` nil hides the text), `.failed(message:)` (muted `footnote` message). Rows 1 and 3 are read in `wordLocale`, the translation in `translationLocale`. Identifiers: `identifier` on the container (`.contain`), `<identifier>.word`, `.ipa`, `.translation`, `.grammar`, `.loading`, `.failure`. Place it with `TranslationBubblePlacement(anchor: wordRect, topLimit: .navTop + .controlH) { bubble }` in a full-screen `.ignoresSafeArea()` layer: centred on the word 10pt above it, clamped `space-4` from the edges, below the word when it would cross `topLimit`.
- Menus from a glass button (the screens' `role="menu"`, no arrow): `.glassMenu(isPresented: $isShown, alignment: .topTrailing) { GlassMenu { rows }.padding(…) }` on the screen's root view; pad the menu to its place (Library: `.padding(.top, .controlH + .space2)`, `.padding(.trailing, .space5)`). Draws a clear layer that closes the menu on a tap outside, scales the menu in from `alignment`, makes it modal for VoiceOver with an escape action. `GlassMenu { … }` is 240pt wide, 4pt inset, `.glassEffect` tinted `surface-glass-strong` in `radius-xl`, `hairline` between rows. Rows: `GlassMenuItem(Text("Select Books"), icon: .select) { … }` (46pt, `body` `ink`, 20pt icon right), `GlassMenuOption(Text("Title"), isSelected: true) { … }` (18pt `accent` check right, selected trait), `GlassMenuHeader(Text("Sort by")) { … }` (40pt submenu header with back chevron, `subhead` at 600 `ink-muted`). Give the button `isActive` while the menu is shown.
- `GlassToolbar { GlassToolbarItem(Text("Remove"), icon: .trash, role: .destructive) { … } }`: the floating bottom toolbar of select mode (Library-Select), a 56pt glass capsule with a 6pt inset and the items spread evenly. `GlassToolbarItem(_:icon:role:action:)`: 18pt icon and `subhead` at 600 side by side, at least 44pt high with `space-3` side padding, `ink`, `danger` for `.destructive`. `.disabled` on the toolbar disables every item. The feature places it (Library: `.safeAreaBar(edge: .bottom)`, `space-5` from the screen sides).
