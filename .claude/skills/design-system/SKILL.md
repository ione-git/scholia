...
- `Icon`: line icons drawn from the screens' SVG paths on the 24pt grid (`back`, `add`, `bookmark`, `more`, `chevron`, `check`, `settings`, `search`). A new icon is a new case with the path from the screen.
...
- `SearchField(text: $query, prompt: Text("Search"), label: Text("Search languages"))`: 40pt `control-fill` field, `radius-md`, 18pt search icon and prompt in `ink-muted`; a search field for XCUITest (`app.searchFields[…]`).
