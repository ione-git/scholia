...
- `Icon`: line icons drawn from the screens' SVG paths on the 24pt grid (`back`, `add`, `bookmark`, `bookmarkFilled` (filled and stroked), `more`, `chevron`, `check`, `settings`, `search`, `select`, `newCollection`, `sort`, `contents`, `highlighter`). A new icon is a new case with the path from the screen.
...
- `LazyGroupedList(items, id: \.index) { item in row }`: the `GroupedList` card for long or unbounded lists (a `LazyVStack`, hairline between rows); give it `.shadow(.card, in: RoundedRectangle(cornerRadius: .radiusLg))` on paper-like screens (Contents). `ChapterRow(title:page:isCurrent:)`: a Contents row, 56pt minimum (grows when the title wraps), 3x22pt `accent` bar (clear when not current), `list-serif` title (600 when current), `subhead` `ink-muted` start page (hidden from VoiceOver: the app sets the accessibility value, e.g. "Page 19", and the selected trait). Wrap it in a `.plain` `Button` like `ListRow`.
