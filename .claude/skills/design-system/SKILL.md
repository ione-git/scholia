...
- Text styles: `.textStyle(.body)` sets font, tracking, exact line height and uppercase (`labelCaps`). `TextStyle.readingBody.lineHeight` is 27; `.font` / `.uiFont` where an API needs a font. Serif styles use the bundled Literata roman at the token weight; sizes are fixed (no Dynamic Type). `.weighted(_:)` takes another style's weight when a screen uses a style's size at a different weight, e.g. 15 regular `ink-muted` secondary text is `TextStyle.callout.weighted(TextStyle.body.weight)`.
...
- Reading scale: `ReadingSize.steps` (7 steps from `type.readingScale` in `tokens.json`; step 3 is `reading-body`), each with `fontSize` and whole-point `tightLineHeight` / `normalLineHeight` / `looseLineHeight`.
...
- Lists for tooling: `ColorToken.all`, `TextStyle.all`, `NumberToken.spacing` / `.radius` / `.effects`, `ShadowToken.all`. The Debug-only Token Gallery (long-press the Home wordmark → Token Gallery, `App/Sources/TokenGallery.swift`) shows them all.

Components (Debug-only Component Gallery: long-press the Home wordmark → Component Gallery, `App/Sources/ComponentGallery.swift`, every component in light and dark). User-facing text is passed in as `Text` from the app, so it stays in the app's String Catalog.

- `GlassButton(.back, label: Text("Back"), size: .regular, isActive: false) { … }`: 44pt glass circle, `.reader` is 48pt. `isActive` draws the open state (`ink` fill, `on-ink` icon, no glass) and adds the selected trait. `GlassButton.Size.reader.diameter` for layout next to it.
- `Icon`: line icons drawn from the screens' SVG paths on the 24pt grid (`back`, `add`, `bookmark`, `bookmarkFilled` (filled and stroked), `more`, `chevron`, `check`, `settings`, `search`, `select`, `newCollection`, `sort`, `contents`, `highlighter`, `lineSpacingTight`, `lineSpacingNormal`, `lineSpacingLoose`, `rotationLock`). A new icon is a new case with the path from the screen.
- `Chip(Text(name), count: 5, isSelected: true) { … }` and `NewCollectionChip(label: Text("New collection")) { … }`. The scrolling row and hiding it without collections are the feature's. `FontChip(Text(verbatim: "Charter"), font: font, isSelected: true) { … }`: the reader settings font chip, at least 40pt high, 16pt side padding, label in the `Font` passed in (build it once), same fills as `Chip`.
- `ThemeSwatch(.paper, title: Text("Paper"), label: Text("Paper theme"), isSelected: true) { … }`: 48pt page-colour circle with a serif "Aa" in the theme's text colour, `caption` name under it (600 `ink` when selected, else `ink-muted`); selected draws a 2pt `accent` ring outside a 2pt `surface-card` gap, at rest a hairline `control-border` ring.
- `TextSizeButton(.smaller, label: Text("Smaller text")) { … }` / `.larger`: 44pt `surface-card` circle with a hairline `control-border` and a small or large serif "A".
- `Toggle(isOn:) { Text("Lock rotation") }.toggleStyle(.icon(.rotationLock))`: 44x40pt `radius-md` icon toggle; on is `ink` fill with an `on-ink` icon, off is `control-fill` with an `ink` icon. VoiceOver and XCUITest see a switch.
- `BookCover(title:author:color:image:size:isFinished:finishedValue:)`: sizes `.thumbnail` 40x60, `.row` 80x120, `.grid` 100x150, `.library` 107x152, `.hero` 160x240 and `.heroLarge` 180x270 (hero glow), `Size.width` / `.height` for layout. `color` is the generated block and the hue of the hero glow, so pass it with an `image` too. A title word too long for the cover shrinks the title instead of breaking mid-word. The cover is one accessibility element labelled with the title; `finishedValue` (e.g. `Text("Finished")`) is its accessibility value while `isFinished`. `ProgressBar(value:)` (0…1) is the Home bar: give it the cover's width. `BookCover.generatedColor(for: title)` is the block colour for a book without cover art: one of the screens' cover colours, stable per title.
- `GoalRing(value:)` (0…1): the Home header ring, 32pt box, `track` ring with an `accent` arc from the top; accessibility value is the percentage, the app adds the label.
- `GroupedSection(Text("Translation")) { rows }` (`label-caps` header + list), `GroupedList { rows }` (`surface-card`, `radius-lg`, hairline between rows), `ListRow(Text("Translate to"), height: .regular) { trailing }` (`.regular` 50pt, `.control` 56pt for a row holding a segmented control), `ListRowValue(Text("Russian"))` (value + chevron), `ListRowChevron()`, `ListRowCheckmark()` (18pt `accent` check of the chosen row in a single-choice list), `ListRowCheckbox(isChecked:)` (24pt circle of a multi-choice list: `accent` fill with an `on-accent` check, or a 1.5pt `ink-faint` ring), `ListActionRow(Text("New Collection…"), icon: .add)` (50pt `accent` action with a 20pt icon, `body` at 500; alone in its own `GroupedList`). A tappable row is a `Button` or `NavigationLink` with `.buttonStyle(.plain)` around a `ListRow`. `ListTextField(Text("Title"), text: $title)`: an editable 50pt row, `ink-muted` label in an 84pt column, `body` input, no placeholder; put the identifier on it and it lands on the text field.
- `SearchField(text: $query, prompt: Text("Search"), label: Text("Search languages"))`: 40pt `control-fill` field, `radius-md`, 18pt search icon and prompt in `ink-muted`, `body` text in `ink`, caret `selection-handle`; a search field for XCUITest (`app.searchFields[…]`). The identifier set on it lands on the text field.
- `Button("Add to Library") { … }.buttonStyle(.solid)`: full-width 52pt `ink` pill with `title-3` `on-ink` label; dims when disabled. `.solid(.compact)` hugs its label: 48pt high, `space-6` side padding, `body` at 600 (the Home-Empty "Add a Book" pill).
- `CoverPlaceholder(label: Text("Add a book")) { … }`: 160x240 hero-cover-sized button, 1.5pt dashed `ink-faint` border at the hero radius, 32pt `ink-faint` plus (Home-Empty).
- `SegmentedControl(selection:size:segments:)` with `.init(value, title:count:identifier:)` or icon segments `.init(value, icon:label:identifier:)` (20pt `ink` icon, `label` is the accessibility label): `.regular` fills the width with equal 36pt segments, `.compact` hugs its labels at 30pt. Selected segment: `surface-card` with a `control-border` shadow at 0 1pt, 3pt blur (the screens' `0 1px 3px`, so it stays visible in dark), title at 600 (`subhead` weighted to `title-3`), count stays 500.
- Sheets: `.modalSheet(isPresented: $isShown) { content }` in place of `.sheet` for modal sheets (`surface`, `radius-sheet`, 36x5pt `track` grabber 8pt below the top edge in place of the system drag indicator; `scrim` over the view it is attached to instead of the iOS dimming, a tap on it dismisses, and that view is hidden from accessibility while the sheet is up). The scrim covers only that view, so attach it to the screen's root view. `.glassSheetStyle()` for the reader settings sheet (system glass, the same grabber, no dimming, the page stays interactive). `SheetHeader(Text("Add Book")) { Button("Cancel") { … }.buttonStyle(.sheetCancel) } trailing: { Button("Done") { … }.buttonStyle(.sheetDone) }` (`EmptyView()` for a missing side); `.sheetBack` is the `accent` chevron + "Back" of a screen pushed inside a sheet.
- `StackedTitle(title:subtitle:titleIdentifier:subtitleIdentifier:)`: centred title `footnote` at 600 `ink` over an optional `caption` `ink-muted` subtitle, 2pt apart, one line each (reader chrome, Contents/Highlights/Bookmarks header).
- Popovers: `.popoverStyle()` on the popover content keeps it a popover on iPhone (system glass).
- Menus from a glass button (the screens' `role="menu"`, no arrow): `.glassMenu(isPresented: $isShown, alignment: .topTrailing) { GlassMenu { rows }.padding(…) }` on the screen's root view; pad the menu to its place (Library: `.padding(.top, .controlH + .space2)`, `.padding(.trailing, .space5)`). Draws a clear layer that closes the menu on a tap outside, scales the menu in from `alignment`, makes it modal for VoiceOver with an escape action. `GlassMenu(size: .regular) { … }` is 240pt wide with 46pt rows (`.reader`: 260pt, 48pt rows, the reader "···" menu), 4pt inset, `.glassEffect` tinted `surface-glass-strong` in `radius-xl`, `hairline` between rows. Rows: `GlassMenuItem(Text("Select Books"), icon: .select) { … }` (`body` `ink`, 20pt icon right), `GlassMenuItem(Text("Themes & Settings"), sample: Text(verbatim: "Aa")) { … }` (serif 17/500 type sample right, hidden from VoiceOver), `GlassMenuDivider()` (6pt `control-fill` band across the menu, no hairlines next to it), `GlassMenuOption(Text("Title"), isSelected: true) { … }` (18pt `accent` check right, selected trait), `GlassMenuHeader(Text("Sort by")) { … }` (40pt submenu header with back chevron, `subhead` at 600 `ink-muted`). Give the button `isActive` while the menu is shown.
