...
- `Icon`: line icons drawn from the screens' SVG paths on the 24pt grid (`back`, `add`, `bookmark`, `bookmarkFilled` (filled and stroked), `more`, `chevron`, `check`, `settings`, `search`, `select`, `newCollection`, `sort`, `collection`, `info`, `trash`, `contents`, `highlighter`). A new icon is a new case with the path from the screen. `Icon.info.menuImage` is the icon as a 20pt template `Image` with the 2pt stroke, for system menus that draw only images (`.contextMenu`): `Label { Text("Book Info") } icon: { Icon.info.menuImage }`.
